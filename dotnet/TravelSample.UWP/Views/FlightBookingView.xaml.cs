...
// but a non-abstract class throws an exception in MvvmCross
        abstract 
#endif
        class FlightBookingPageAbstract : MvxWindowsPage<FlightBookingViewModel>
    {}

    /// <summary>
    /// A view that shows a list of flight bookings that the user has created
    /// </summary>
    public sealed partial class FlightBookingView : FlightBookingPageAbstract
    {
        #region Constructors
        ...
        {
            // Make sure the correct row is passed to the view model
            var listView = (ListView) sender;
            CellFlyout.Items[0].DataContext = ((FrameworkElement) e.OriginalSource).DataContext;
            (CellFlyout.Items[0] as MenuFlyoutItem).Command = ViewModel.RemoveBookingCommand;

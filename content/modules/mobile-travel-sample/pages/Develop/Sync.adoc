...
In the lesson on link:/swift#/2/2/1[Access Control] we discussed how the Couchbase Sync Gateway supports Authorization and Access Control functions.
...
Before you begin this lesson, confirm that you have the Sync Gateway up and running by following the instructions in the link:/tutorials/travel-sample/develop/swift#/0/4/0["Installation"] section. 

Starting with Sync Gateway 1.5 and Couchbase Server 5.x, mobile and server/web applications now have the ability to read and write to the same bucket.
...
In the Travel app, the list of users that are permitted access is configured in the Sync Gateway configuration file as discussed in the link:/tutorials/travel-sample/develop/swift#/2/2/1[Access
  Control] section 
...
The list of channels that the user has access to is defined in the Sync Gateway configuration file as discussed in the link:/tutorials/travel-sample/develop/swift#/2/3/0[Channels/
